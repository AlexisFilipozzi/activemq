...
class Init():
    ...
    def do_setting_activemq_jmx_access(self, role, user, password):
        ...
        'ACTIVEMQ_OPTS="$ACTIVEMQ_OPTS -Djava.rmi.server.hostname=' + self.get_rmi_server_hostname() + ' -Dcom.sun.management.jmxremote.port=1099 -Dcom.sun.management.jmxremote.rmi.port=1099 -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=true -Dcom.sun.management.jmxremote.password.file=' + ACTIVEMQ_CONF + '/jmx.password -Dcom.sun.management.jmxremote.access.file=' + ACTIVEMQ_CONF + '/jmx.access"')

    def get_rmi_server_hostname(self):
        return os.getenv("JAVA_RMI_SERVER_HOSTNAME", "0.0.0.0")
